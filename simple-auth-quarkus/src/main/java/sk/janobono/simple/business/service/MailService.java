...
public class MailService {
  ...
  public void sendEmail(final MailData mailData) {
    try {
      ...
      Optional.ofNullable(mailData.attachments())
          ...
          .flatMap(Collection::stream)
          .forEach(attachment -> {
            mail.addAttachment(attachment.getKey(), attachment.getValue(),
                "application/octet-stream");
          });

      mailer.send(mail);
    } finally {
      Optional.ofNullable(mailData.attachments()).map(Map::values).stream()
          .flatMap(Collection::stream)
          .forEach(attachment -> {
  ...
  private String renderEmail(final MailContentData mailContentData) {
    return this.email
        ...
        .data("linkHref",
            Optional.ofNullable(mailContentData.mailLink()).map(MailLinkData::href).orElse(""))
        .data("linkText",
            Optional.ofNullable(mailContentData.mailLink()).map(MailLinkData::text).orElse(""))
        .render();
  }
}
